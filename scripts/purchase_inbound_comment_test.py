def get_erp_purchase_inbound_test():
    data = frappe.db.sql("""
        SELECT
            po.name AS po_no,
        ...
        /* ✅ SAFE comment join (1 row per PO) */
        LEFT JOIN (
            SELECT
                c1.reference_name,
                TRIM(
                    REGEXP_REPLACE(
                        REGEXP_REPLACE(c1.content, '<[^>]*>', ''),
                        '&nbsp;', ' '
                    )
                ) AS latest_comment,
                c1.creation AS comment_time,
                c1.owner AS commented_by
            FROM `tabComment` c1
            INNER JOIN (
                SELECT
                    reference_name,
              ...
              ON c1.reference_name = c2.reference_name
             AND c1.creation = c2.max_creation
        ) lc ON lc.reference_name = po.name

        WHERE
            po.is_subcontracted = 0
            AND po.status NOT IN ('Closed', 'Completed', 'Cancelled', 'Draft')
            AND po.supplier_name NOT IN ('Palaniyappa Traders')
        ...
        GROUP BY
            ...
            lc.latest_comment, lc.comment_time, lc.commented_by
        ...
        LEFT JOIN `tabSubcontracting Order Item` soi
            ON soi.parent = so.name AND soi.item_code = poi.item_code
        LEFT JOIN `tabSubcontracting Receipt Item` sor
        ...
        ORDER BY po_no, po_date
    """, as_dict=1)

    return data

frappe.response["message"] = get_erp_purchase_inbound_test()
