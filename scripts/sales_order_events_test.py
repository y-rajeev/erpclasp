"""
Server Script (readability export): Sales Order Events Test
script_type: API
api_method: get_sales_order_events_test

Source of truth for backup/import is the sibling .json next to this folder.
"""
def get_sales_order_events_test():
    ...
    query = f"""
        SELECT
            ...
            CASE
                WHEN i.custom_item_type = 'Stock' AND events.sku NOT LIKE 'FA-%%' THEN 'Ready Goods'
                WHEN i.custom_item_type = 'Stock' AND events.sku LIKE 'FA-%%' THEN 'Fabric'
                WHEN i.custom_item_type = 'Assembly' THEN 'Ready Goods'
                WHEN i.custom_item_type = 'Manufacture' THEN 'Manufacture'
                ELSE NULL
            END AS product_type,
...
# API response
frappe.response["message"] = get_sales_order_events_test()
