def get_purchase_inbound():
    data = frappe.db.sql("""
        ...
        GROUP BY
            ...
            po.schedule_date, poi.item_code, poi.item_name, poi.qty, po.set_warehouse
        ...
        SELECT
            ...
            poi.custom_design AS item_name,
            poi.qty AS required_qty,
        ...
        GROUP BY
            po.name, po.transaction_date, po.supplier_name, w.branch,
            po.schedule_date, poi.fg_item, poi.custom_design, poi.qty, po.set_warehouse
...
frappe.response["message"] = get_purchase_inbound()
